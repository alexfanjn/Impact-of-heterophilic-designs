...
class H2GNN(torch.nn.Module):
    def __init__(self, data, num_features, num_hidden, num_classes, dropout, layer_num=2, device='cpu'):
        ...
        sparse_adj_tensor = edge_index_to_sparse_tensor_adj(temp_loop_edge_index, self.data.x.shape[0])
    ...
    def forward(self, data):
        ...
        self.norm_adjs = []
        self.norm_adjs.append(gcn_norm(data.edge_index, self.data.y.shape[0], self.device))
        self.norm_adjs.append(gcn_norm(self.k_hop_edge_index[0], self.data.y.shape[0], self.device))


        h = self.linear1(data.x)
        h = F.relu(h)
        final_h = h
...
class H2GNN_Basic(torch.nn.Module):
    def __init__(self, data, num_features, num_hidden, num_classes, dropout, layer_num=2, device='cpu'):
        super(H2GNN_Basic, self).__init__()

        self.linear1 = torch.nn.Linear(num_features, num_hidden[0])

        self.linear2 = torch.nn.Linear(2 * num_hidden[0] + 4 * num_hidden[0], num_classes)
